<?php
	...
	$name_error = $email_id_error = $phone_number_error  = $employee_id_error = "";
	$name= $email_id = $phone_number = $employee_id = "";
	
	if (isset($_POST["submit"])) {
		$name = check_data($_POST["name"]);
		$email_id = check_data($_POST["email_id"]);
		...
		if ($email_id == "") {
			$email_id_error = "email_id is required";
			$email_id = "";
			} else {
		}
		if ($phone_number == "") {
		...
		if($name_error == "" && $email_id_error == "" && $phone_number_error == "" && $employee_id_error == "" )
		{
			...
			$dbname = "bloodbank";
			$connection = mysqli_connect($dbhost , $dbuser, $dbpass, $dbname);
			$query = "insert into manager (name, email_id,phone_number,employee_id) 
			values ('{$name}', '{$email_id}' , '{$phone_number}' , '{$employee_id}');";
    ...
    <!---------------------------------Registration Form-------------------------------------->
		...
		<?php //the value of each field is going to come from $variables , so that valid fields are not gone if
			...
			<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
				...
				<br /><br />
				email_id : 
				<input type = "VARCHAR" name = "email_id" value="" maxlength="25" size = "30px">
